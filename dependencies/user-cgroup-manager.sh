...
# Iterate over each matching username and perform actions
echo "Iterating over matching usernames:"
for username in "${validusers[@]}"; do
    for pid in $(pgrep -u $username); do
        apply_limits "$username" "$pid"
    done
done
