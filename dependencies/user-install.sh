...
if [ -z "$cpu" ] || [ -z "$memory" ] || [ -z "$swapmem" ] || [ -z "$disk" ] || [ -z "$contract_dir" ] ||
    ...
    [ -z "$debug_shell_uid" ] || [ -z "$debug_shell_gid" ]; then
...
# Creating a secondary user for the contract and debug_shell.
...
debug_shell_host_uid=$(expr $uoffset + $debug_shell_uid - 1)
...
# Even though there's this "if not 0" condition, contract_gid and debug_shell_gid will always be 0 since we are setting hp config's gid to 0 in instance creation.
...
# Do debug_shell user creation same as above.
if [ ! $debug_shell_gid -eq 0 ]; then
    debug_shell_host_gid=$(expr $goffset + $debug_shell_gid - 1)
    groupadd -g "$debug_shell_host_gid" "$debug_shell_user"
    useradd --shell /usr/sbin/nologin -M -g "$debug_shell_host_gid" -G "$user" -u "$debug_shell_host_uid" "$debug_shell_user"
else
    debug_shell_host_gid=$(id -g "$user")
    useradd --shell /usr/sbin/nologin -M -g "$debug_shell_host_gid" -u "$debug_shell_host_uid" "$debug_shell_user"
fi

usermod --lock "$debug_shell_user"
echo "Created '$debug_shell_user' debug_shell user."
