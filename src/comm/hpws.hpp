...
#define HPWS_LONG_TIMEOUT 1500 // This timeout has to account the possible delays in communication via internet.
    ...
    private:
        ...
        client(
            std::string_view get,
            addr_t endpoint,
            int control_line_fd_0, // hpws -> sagent  [ hpws sends bufs to us over this line ]
            int control_line_fd_1, // sagent -> hpws  [ we send bufs to hpws over this line  ]
            uint32_t max_buffer_size,
            pid_t child_pid,
            int buffer_fd[4],
...
#define HPWS_CONNECT_ERROR(code, msg) \
    {                                 \
        ...
        goto connect_error;           \
            ...
            int fd[4] = {-1, -1, -1, -1}; // 0,1 are hpws->sagent, 2,3 are sagent->hpws
...
#define HPWS_ACCEPT_ERROR(code, msg)                       \
    {                                                      \
        ...
        return error{code, msg};                           \
            ...
            pfd.fd = child_fd[0]; // expect all setup messages on the hpws->sagent controlfd (0)
            pfd.events = POLLIN;
